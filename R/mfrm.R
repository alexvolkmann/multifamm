...
# Note:
# Data has to be of the following format
# y_vec       : numeric, vector of response values
# t           : numeric, observations point locations
...
# covariate.1 : numeric, potential covariate(s) specified with 1,2,3,...

# Note:
# MFPCA package version 1.1 needed - has to be downloaded from github
...
#' @export
mvfr <- function(data, fRI_B = FALSE, fRI_C = FALSE, bs = "ps",
  ...
  #                       potential heteroscedasticity ("w_bam", "bam", "gamm",
  #                       "gaulss")
...
approxNA <- function (object) {
  ...
  else warning("Package zoo needed for interpolating missing values.")
}
#------------------------------------------------------------------------------#
...
final_model <- function(formula, data, final_method, model_list){
  ...
  switch(final_method,
    ...
    "bam" = bam(formula = formula, data = data, discrete = TRUE),
    ...
    "w_bam" = {
      ...
      bam(formula = formula, data = data, weights = norm_weights,
          discrete = TRUE)
    },
    ...
    "gamm" = gamm(formula = formula, data = data,
    ...
    "gaulss" = gam(formula = list (formula, ~ 0 + dim), data = data,
                   family = gaulss())
    )
